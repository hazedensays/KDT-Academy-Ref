...
const mockTodo = [
  ...
  { id: 1, isDone: true, content: "JavaScript 공부하기", createDate: new Date().getTime() },
  { id: 2, isDone: false, content: "CSS 공부하기", createDate: new Date().getTime() },
  { id: 3, isDone: false, content: "Html 예습하기", createDate: new Date().getTime() },
  { id: 4, isDone: false, content: "Java 예습하기", createDate: new Date().getTime() }
...
function App() {
  ...
  // => 상태값 변경 부분을 reducer()로
  // dispatch 호출해서 action(type, data)값을 전달해주어야 함
  ...
  const onUpdate = useCallback(
    (targetId) => { dispatch({ type: "Update", targetId });
  ...
  const onDelete = useCallback(
    (targetId) => { dispatch({type: "Delete",targetId });}
    , []);
