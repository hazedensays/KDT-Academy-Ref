...
public class Ex01_Flow01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// => 1-1) Forward : Servlet -> Servlet (to helloS)
		String uri = "helloS";
		// RequestDispatcher dis = request.getRequestDispatcher(uri);
		// dis.forward(request, response); => 아래 코드 풀어쓰기
		// request.getRequestDispatcher(uri).forward(request, response);

		// => 1-2) Forward : Servlet -> JSP (to form04_Select.jsp)
		uri = "servletTestForm/form04_Select.jsp";
		request.getRequestDispatcher(uri).forward(request, response);

		// 2. Redirect (재요청 처리)
		// 첫번째 요청 flow01 에서 helloS 라는 요청을 다시 보내줄것을 웹브라우저에게 response 함.
		// 재요청으로 helloS가 서버로 전달되어 처리하는 방식
		// => 그러므로 웹 브라우저에 표시된 요청명이 hellos 로 변경됨
