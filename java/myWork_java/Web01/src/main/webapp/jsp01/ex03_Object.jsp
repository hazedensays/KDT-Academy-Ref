...
<h3>3. out</h3>
* Session_ID => <% out.print("~~~out 객체로 출력하기"); %>
...
<%HttpServletRequest req = (HttpServletRequest)pageContext.getRequest();%>
	-> 비교 결과 : <%=request==req%>
	-> pageContext가 제공하는 out객체로 출력하기
	<% pageContext.getOut().print("~~pageContext가 제공하는 out객체로 출력~~"); %>
	
<h3>5. Application</h3>
* 서버정보 => <%=application.getServerInfo()%>
* 서블릿 메이저버전 => <%=application.getMajorVersion()%>
* RealPath => <%=application.getRealPath("") %>
   - ""사용시에는 현 프로젝트의 실행중인 RealPath return
* RealPath2 => <%=application.getRealPath("/jsp01") %>
   - 경로 입력하면 해당 경로의 real path가 출력됨
   - "/jsp01" 입력함
