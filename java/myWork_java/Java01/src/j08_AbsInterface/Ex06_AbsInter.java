...
class MyBoard implements Boardi {
	...
	@Override
	public void replyInsert() {
		System.out.println("** MyBoard replyInsert **");
	}
...
class QnABoard extends BoardA {
	...
	@Override
	public void listPrint() {
		System.out.println("** QnABoard List **");
	}
...
class NoticeBoard extends BoardA {
	...
	@Override
	public void listPrint() {
		System.out.println("** NoticeBoard List **");
	}
}
...
public class Ex06_AbsInter {
	...
	public static void main(String[] args) {
		...
		// => BoardA를 상속받지 않은 MyBoard로는 생성할 수가 없음
		ba.listPrint();
		ba.delete();
		// => QnABoard에는 정의하지 않았지만, 조상인 BoardA에 있으므로 가능
		// => 그러나 BoardA에는 없고, QnABoard에만 정의한 메서드가 있다면 사용 불가능
